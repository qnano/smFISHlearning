function [coords,dectectionPar,cutProcess] = LLRMap3D(process,PSFSigma,minPixels,compReduction,significance,iterations,split,maxCudaFits, mask)
...
% 
%     compReduction:  level in standard 
...
% coords570=[coordspre(:,1) coordspre(:,2) coordspre(:,3)];
    ...
    if ~isempty(compReduction)
        ...
        locIm =  W{2}.*mask(szx,szy,szzet)	>mean(dip_image(W{2}),[],[1 2])+compReduction*std(dip_image(W{2}),[],[1 2]);
